...
- 多模型适配：通过 LiteLLM 兼容 OpenAI-Compatible、DeepSeek、Qwen、Ollama 等供应商。
- 工作流编排：为 review、testgen、chat 预留 LangChain / LangGraph 扩展点。

## 技术栈

- 后端：FastAPI、SQLAlchemy 2.x、Alembic、Pydantic、Celery
- AI 编排：LangChain、LangGraph、LiteLLM
- 检索：SQLite FTS / FAISS，部署路径可扩展到 PostgreSQL + pgvector
...
默认配置中 `ENABLE_MOCK_LLM=true`，可以在不接入真实大模型的情况下先启动和体验主要流程。接入真实模型时，按需设置 `LLM_BASE_URL`、`LLM_API_KEY`、`LLM_CHAT_MODEL` 和 `LLM_EMBEDDING_MODEL`。
